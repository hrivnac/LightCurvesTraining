...
from parse_ztf import canonicalize_labels
from parse_ztf import split_semicol
from parse_ztf import split_commas
...
def threshold_sweep(y_true, y_prob, thresholds=(0.01, 0.02, 0.05, 0.1, 0.2, 0.3)):
    ...
    for thr in thresholds:
        ...
        rows.append(
            (thr, int(tp), int(fp), int(fn), float(prec), float(rec), float(f1))
        )
    return rows
...
def _pad_to_tmax(
    x: tf.Tensor, mask: tf.Tensor, tmax: int
...
def make_eval_dataset(
    objects: List[dict], num_labels: int, batch_size: int, tmax: int
) -> tf.data.Dataset:
    def gen():
        for o in objects:
    ...
    ds = ds.map(
        lambda t, mag, band, y: _to_model_inputs(t, mag, band, y, tmax),
...
def main():
    ...
    ap.add_argument(
        "--label2id", required=True, help="Path to label2id.json saved during training"
    )
    ap.add_argument("--batch", type=int, default    ap.add_argument("--data", required=True, help="New dataset CSV (same format)")
=256, help="Batch size for evaluation")
    ap.add_argument(
        "--tmax", type=int, default=64, help="Max padded length (must match training)"
    )
    ap.add_argument(
        "--threshold", type=float, default=0.5, help="Threshold for micro-F1"
    )
    ap.add_argument(
        "--out", default="", help="Optional: write per-object top-K predictions CSV"
    )
    ap.add_argument(
        "--topk", type=int, default=10, help="Top-K labels to export per object"
    )
    args = ap.parse_args()
    ...
    # NOTE: Use custom_objects for custom layers.
    ...
    print("Parsing data:", TEST_CSV)
    objects = load_objects_from_csv(TEST_CSV, label2id)
    print("Objects:", len(objects), "Labels:", num_labels)

    ds = make_eval_dataset(
        objects, num_labels=num_labels, batch_size=args.batch, tmax=args.tmax
    )
    ...
    for thr, tp, fp, fn, prec, rec, f1 in threshold_sweep(y_true, y_prob):
        print(
            f"thr={thr:>5}: tp={tp} fp={fp} fn={fn}  prec={prec:.3f} rec={rec:.3f} f1={f1:.3f}"
        )
    ...
    out_table = os.path.join(
        os.path.dirname(args.out) if args.out else ".", "eval_per_class.csv"
    )
